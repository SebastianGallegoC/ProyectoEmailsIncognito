<template>
  <div class="space-y-6">
    ...
    <div v-else-if="contacts.length === 0"
      class="bg-[#292A2D] rounded-2xl shadow-xl p-12 border border-[#3C4043] text-center">
      ...
      <button @click="showCreateModal = true" class="text-[#8AB4F8] hover:text-[#669DF6] font-medium">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="contact in contacts" :key="contact.id"
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="flex items-center gap-2 mt-1">
                <button @click="toggleFavorite(contact)" class="transition-colors"
                  :class="contact.isFavorite ? 'text-yellow-400' : 'text-[#9AA0A6] hover:text-yellow-400'">
                  <svg class="w-4 h-4" :fill="contact.isFavorite ? 'currentColor' : 'none'" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                  </svg>
                </button>
                <span v-if="contact.isBlocked" class="text-[#F28B82] text-xs font-medium">
                  Bloqueado
                </span>
              </div>
            </div>
        ...
        <div class="flex gap-2">
          ...
          <button @click="toggleBlock(contact)"
    ...
    <Teleport to="body">
      <div v-if="showCreateModal || showEditModal"
        class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" @click.self="closeModal">
