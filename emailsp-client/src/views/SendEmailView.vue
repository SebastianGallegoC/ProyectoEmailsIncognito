<template>
  <div class="space-y-6">
      ...
      <form @submit.prevent="handleSendEmail" class="space-y-6">
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            ...
            <button type="button" @click="showContactPicker = true"
          ...
          <div class="space-y-2">
            <div v-for="(recipient, index) in recipients" :key="index" class="flex gap-2">
              <input v-model="recipients[index]" type="email" required placeholder="destinatario@example.com"
              ...
              <button v-if="recipients.length > 1" type="button" @click="removeRecipient(index)"
        ...
        <div>
          ...
          <input id="subject" v-model="subject" type="text" required placeholder="Ingresa el asunto del correo"
        ...
        <div>
          ...
          <textarea id="body" v-model="body" rows="16" required placeholder="Escribe tu mensaje aquí... (Soporta HTML)"
            class="w-full px-4 py-3 bg-[#3C4043] border border-[#5F6368] rounded-lg text-[#E8EAED] placeholder-[#9AA0A6] focus:outline-none focus:ring-2 focus:ring-[#8AB4F8] focus:border-transparent resize-none"></textarea>
    ...
    <Teleport to="body">
      <div v-if="showContactPicker" class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4"
        @click.self="showContactPicker = false">
        <div
          class="bg-[#292A2D] rounded-2xl shadow-2xl border border-[#3C4043] max-w-2xl w-full max-h-[80vh] flex flex-col">
          ...
          <div class="flex-1 overflow-y-auto p-6">
            ...
            <div v-else-if="contacts.length === 0" class="text-center py-8">
              <svg class="w-16 h-16 text-[#9AA0A6] mx-auto mb-4 opacity-50" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
          ...
          <div class="p-6 border-t border-[#3C4043] flex gap-4">
            <button @click="addSelectedContacts" :disabled="selectedContacts.length === 0"
